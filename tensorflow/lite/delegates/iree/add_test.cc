...
#include "tensorflow/lite/delegates/iree/binary_elementwise_tester.h"
...
TEST(Add, 4DBy4D) {
  std::unique_ptr<TfLiteDelegate, decltype(&TfLiteIreeDelegateDelete)>
      iree_delegate(TfLiteIreeDelegateCreate(nullptr),
  ...
  BinaryElementwiseTester()
      ...
      .Test(BuiltinOperator_ADD, iree_delegate.get());
}
...
TEST(Add, MultiThreading) {
  TfLiteIreeDelegateOptions delegate_options =
      TfLiteIreeDelegateOptionsDefault();
  // delegate_options.num_threads = 2;
  std::unique_ptr<TfLiteDelegate, decltype(&TfLiteIreeDelegateDelete)>
      iree_delegate(TfLiteIreeDelegateCreate(&delegate_options),
                    TfLiteIreeDelegateDelete);
